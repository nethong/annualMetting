...
//     }
    ...
    var t=setInterval(function(){
        ...
        var top = imgs.parent().eq(img_index).position().top;
        var left = imgs.parent().eq(img_index).position().left;
        $(".pop").css({left: Math.ceil(Math.random() * 2000 - 400)});
        $(".pop" + img_index).animate({top: top, left: left, opacity: 1}, 1990, function () {
            imgs.eq(j).attr("src", src).show();
            $(this).find("img").animate({width:11,height:22},990,function(){
                $(this).parent().remove();
            });
        });
        arr_index++;
